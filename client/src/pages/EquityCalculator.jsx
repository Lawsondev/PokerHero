...
export default function EquityCalculator() {
  ...
  const validateInputs = (h, o, b) => {
    ...
    const hc = parse(h),
      oc = parse(o),
      bc = parse(b);
    let errs = { heroHand: '', oppHand: '', board: '', duplicates: '' },
      ok = true;
  ...
  const handleCalculate = () => {
    ...
    const fmtCard = (cs) =>
      cs.trim().length === 2
        ...
        : cs.trim().toUpperCase();
    const fmtHand = (str) => str.trim().split(/\s+/).map(fmtCard).join('');
  ...
  const genHeroHand = () => {
    ...
    const exclude = new Set(
      [...oppHand.split(' '), ...board.split(' ')].filter(Boolean)
    );
    let combo, c1, c2;
    do {
  ...
  const genVillainHand = () => {
    ...
    const exclude = new Set(
      [...heroHand.split(' '), ...board.split(' ')].filter(Boolean)
    );
    let combo, c1, c2;
    do {
      combo = pool[Math.floor(Math.random() * pool.length)];
  ...
  return (
    <div className="p-8 max-w-screen-lg mx-auto font-sans text-gray-800">
      ...
      <p className="mb-6 text-gray-700 leading-relaxed">
        This tool lets you calculate the equity of one hand versus another in
        real-time. Enter hole cards for both players and optionally fill out the
        board; the tool will simulate many outcomes and return a percentage
        equity.
      </p>
      ...
      <div className="mb-6">
        ...
        <p className="mb-2 text-gray-600">
          Either type in each player’s two cards, or click “Generate Random
          Hand.”
        </p>
      ...
      <div className="mb-6">
        ...
        <div className="space-y-4">
          {/* Hero’s Hand */}
          <div>
            <label htmlFor="hero-hand" className="block font-semibold">
              Hero's Hand
            </label>
            {heroHand && (
              <div className="mt-2">
            ...
            />
            {errors.heroHand && (
              <p className="text-red-600 mt-1">{errors.heroHand}</p>
            )}
          </div>

          {/* Opponent’s Hand */}
          <div>
            <label htmlFor="opp-hand" className="block font-semibold">
              Opponent's Hand
            </label>
            {oppHand && (
              <div className="mt-2">
            ...
            />
            {errors.oppHand && (
              <p className="text-red-600 mt-1">{errors.oppHand}</p>
            )}
          </div>

          {/* Board Cards */}
          <div>
            <label htmlFor="board-cards" className="block font-semibold">
              Board Cards
            </label>
            {board && (
              <div className="mt-2">
      ...
      <div className="mb-6">
        ...
        {showExplanation && (
          <p className="mt-2 text-gray-600 text-sm">
            The percentage is based on a real time calculation of total hands
            won vs. total hands lost, during the simulations set above. More
            simulations equals more accuracy.
          </p>
        )}
      </div>
    </div>
